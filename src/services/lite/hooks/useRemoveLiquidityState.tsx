...
const useRemoveLiquidityState: () => RemoveLiquidityState = () => {
  ...
  // tslint:disable-next-line:max-func-body-length
  useAsyncEffect(async () => {
    if (state.selectedLPToken && state.selectedLPToken.totalSupply && state.pair && state.fromToken && state.toToken) {
    ...
    if (state.fromAmount && state.toAmount && state.selectedLPToken && state.amount && signer) {
      ...
      try {
        ...
        if (state.fromSymbol === "WETH" || state.toSymbol === "WETH") {
          ...
          const tx = await removeLiquidity(state.fromToken!, state.toToken!, liquidity, fromAmount, toAmount, signer);
          //await tx.wait();
        }
        await updateTokens();
